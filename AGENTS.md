# AGENTS.md — working agreement for the Hyades engine
...
- **Check liveness by file mtime, never by `pgrep -f <pattern>`** — the pattern
  ...
  and AGENTS.md had already recorded the same trap once (the `pkill -f` incident
...
`AGENTS.md`'s ordering held: ablation refutes, instrumentation explains, and a
...
believe either** — `AGENTS.md`'s "ablate before you explain", applied *before*
...
- **Never squash commits that have been pushed to the remote** (the author's
  ruling). Merge a PR into `main` with a **merge commit**, not a squash or a
  rebase, so the commits a reviewer saw — each with its own message, its own
  measurement and its own T-codes — survive in `main`'s history. The same
  applies before a PR: once a commit is on the remote, later work is a new
  commit on top, never an amend or a squash of it. `main` carries squashed
  merges up to #9; they stay as they are, and the rule applies from here on.
