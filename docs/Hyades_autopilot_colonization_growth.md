...
- **Target selection is fog-limited by construction.** The candidate list handed to `Autopilot::choose_survey_target` is `SurveyView { id, position, habitability, biosphere }` — precisely §1's remote tier — so a survey target can only be chosen on what spectroscopy can see. In particular the list is **not** filtered by ownership: a scout has no way to know a world is claimed until it gets there, so it may well fly to one and learn on arrival. That costs hops, and it is the correct cost. (Worlds this empire has already dispatched to are excluded, so it never re-targets something it has scanned.) Before this was enforced by the type, the engine filtered candidates on ground-truth ownership and handed out mineral density and pop level for unvisited worlds — omniscience on all three counts.
