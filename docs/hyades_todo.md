...
### T-104. Population is not a factor of production — `Y` must read `P`

**The prerequisite under T-103 and under the whole Warfare tree**, and it is a
...
### T-103. Development is a profitability test — R-IND22
...
Specified in `Hyades_industry.md` §1.8. **Blocked on T-104**, which supplies the
...
it on work-years after T-104, not before.

---

### T-105. The write-capability partition — only Warfare may kill population
...
Independent of T-104 as *engine work* — the gate compiles either way — and
dependent on it for the gate to matter.

---

### T-106. The first Warfare card — the armed coloniser

Specified in `Hyades_trees_and_card_value.md` §5, with the roles amendment in
`Hyades_vehicle_roles.md` §4.2. One Design write and three Doctrine writes; the
...
| 4 | Warfare's objective is an algebraic zero on the standard bed | R-TREE8 |
| 5 | population is not a factor of production | **T-104** |
...
### ~~T-90. Outpost selection is colour-blind — R-O91~~ — **IMPLEMENTED AND REFUTED**

> **The term is a real defect and it is not the cause.** `local_scarcity` weighted
> a world's colours by the deciding centre's own shortfall against its next rung,
...
> already mines a balanced mix, a rock is one colour, and a hold is filled from
