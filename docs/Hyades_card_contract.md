...
- **It does not make Warfare cards population-lethal.** It licenses them. The
  first Warfare card specified (trees §6) carries no lethal write at all — its
...
- **It is not a claim that population loss is currently costly.** It is not:
  ...
  and the thing it gates are separate work items (T-105 and T-104).

**R-TREE10** carries the two calls this leaves open: whether `lethal` should be
